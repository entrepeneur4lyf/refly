...
import { genSkillID, genSkillJobID, genSkillTriggerID, modelList } from '@refly/utils';
...
export class SkillService {
  ...
  async skillInvokePreCheck(user: User, param: InvokeSkillRequest): Promise<InvokeSkillJobData> {
    ...
    const modelInfo = modelList.find((model) => model.name === data.modelName);
          ...
          case 'on_chat_model_end':
            if (runMeta && chunk) {
              const modelInfo = modelList.find(
                (model) => model.name === String(runMeta.ls_model_name),
              );
              if (!modelInfo) {
